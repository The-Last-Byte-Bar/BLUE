#!/usr/bin/env python3
"""
Blockchain Analysis Demo Script with Real Data

This script demonstrates the blockchain analysis functionality using LLM integration
with actual blockchain data.

To run this script:
    ./demo_blockchain_analyzer_real.py interactive
    
Example commands:
    ./demo_blockchain_analyzer_real.py wallet 9gUDVVx75KyZ783YLECKngb1wy8KVwEfk3byjdfjUyDVAELAPUN
    ./demo_blockchain_analyzer_real.py interactive
"""
...
from llm.analysis_fixed import (
    analyze_wallet
)

# Import other analysis functions from the original module
...
async def demo_wallet_analysis(address: str, question: Optional[str] = None, provider: str = "claude"):
    """Run a wallet analysis demonstration with real blockchain data."""
    print(f"\n{'=' * 80}")
...
async def interactive_session(initial_address: Optional[str] = None, provider: str = "claude"):
    """Run an interactive analysis session with real blockchain data."""
    print(f"\n{'=' * 80}")
    print(f"INTERACTIVE BLOCKCHAIN ANALYSIS SESSION (REAL DATA)")
    print(f"{'=' * 80}\n")
    
    print(f"Using LLM Provider: {provider}")
    print("Type 'exit' or 'quit' to end the session")
    print("Enter any real Ergo blockchain address to analyze it")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Blockchain Analysis Demo with Real Data")
